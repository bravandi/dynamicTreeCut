...
def cutreeHybrid(link, distM,
    ...
    for merge in range(nMerge):
        if dendro_height[merge] <= cutHeight:
            ...
            if dendro_merge[merge, 0] < 0 and dendro_merge[merge, 1] < 0:
                ...
                sizes = sizes[rnk - 1]
                branch1 = np.nan if np.any(np.isnan(branch_singletons[large - 1])) else branch_singletons[large - 1][
                    np.arange(sizes[1])]
                branch2 = np.nan if np.any(np.isnan(branch_singletons[small - 1])) else branch_singletons[small - 1][
                    np.arange(sizes[0])]
                spyMatch = False
                if spyIndex != None:
                ...
                if SmallerScores[0] * np.sum(SmallerScores[1:]) > 0:
                    DoMerge = True
                    SmallerFailSize = ~np.logical_or(SmallerScores[2],
                                                     SmallerScores[3])  # Smaller fails only due to size
                else:
                    ...
                    if LargerScores[0] * np.sum(LargerScores[1:]) > 0:
                        ...
                        DoMerge = True
                        SmallerFailSize = ~np.logical_or(LargerScores[2],
                                                         LargerScores[3])  # cluster fails only due to size
                        x = small
                        small = large
                        large = x
                        sizes = sizes[::-1]
                    ...
                    for sclust in SmallLabLevs[SmallLabLevs != 0]:
                        ...
                        if len(labelsOnBranch) > 0:
                            if len(InCluster) > 1:
                                ...
                                DistToMeds = get_rows(Medoids[labelsOnBranch - 1][Medoids[labelsOnBranch - 1] != 0] - 1,
                                                      distM)[:, smed]
            ...
            if len(Unlabeled) > 0:
                if pamRespectsDendro:
                    ...
                    for obj in unlabOnBranch:
                        ...
                        UnassdToClustDist = tapply(get_rows(useObjects, distM)[:, obj], useColorsFac,
                                                   np.mean)  ### think about
    ...
    results = dict(labels=OrdNumLabs - UnlabeledExist,
                   ...
                   onBranch=onBranch,
                   mergeDiagnostics=mergeDiagnostics if nExternalSplits == 0 else pd.DataFrame(
                       {'x': mergeDiagnostics, 'y': externalMergeDiags}),
